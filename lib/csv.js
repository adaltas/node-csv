...
var from, options, state, stream, to;
...
module.exports = function() {
  var CSV, csv, error, parse, transform, transforming, write;
  transforming = false;
  CSV = function() {
    this.readable = true;
    this.writable = true;
  ...
  */
  ...
  CSV.prototype.write = function(data, preserve) {
    ...
    if (this.state.count === 0 && csv.options.to.header === true) {
  ...
  CSV.prototype.end = function() {
    ...
    if (csv.writeStream) {
      if (this.state.bufferPosition !== 0) {
        csv.writeStream.write(this.state.buffer.slice(0, this.state.bufferPosition));
      }
      if (this.options.to.end) {
        return csv.writeStream.end();
      } else {
        csv.emit('end', this.state.count);
