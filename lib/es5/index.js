  ...
  function Parser() {
    ...
    if (options.comment === undefined || options.comment === null || options.comment === false || options.comment === '') {
      ...
      if (!Buffer.isBuffer(options.comment)) {
        throw new Error("Invalid Option: comment must be a buffer or a string, got ".concat(JSON.stringify(options.quote)));
      }
  ...
  _createClass(Parser, [{
    ...
    value: function __parse(nextBuf, end) {
      var _this$options = this.options,
          ...
          max_limit_on_data_read = _this$options.max_limit_on_data_read,
      ...
      for (pos = 0; pos < bufLen; pos++) {
        ...
        if (commenting === false && max_limit_on_data_read !== 0) {
          if (this.state.field.length > max_limit_on_data_read) {
            var _err3 = this.error("Field exceeds max_limit_on_data_read: maximum value is ".concat(max_limit_on_data_read));

            if (_err3 !== undefined) return _err3;
          }

          if (this.state.record_length + this.state.field.length > max_limit_on_data_read) {
            var _err4 = this.error("Record exceeds max_limit_on_data_read: maximum value is ".concat(max_limit_on_data_read));
        ...
        if (lappend === true && rappend === true) {
          ...
          var _err5 = this.error("Invalid Closing Quote: found non trimable byte after quote at line ".concat(this.info.lines));
      ...
      if (end) {
        if (this.state.quoting === true) {
          var _err6 = this.error("Invalid Closing Quote: quote is not closed at line ".concat(this.info.lines));
      ...
      if (columns === false && this.info.records === 0) {
        ...
        if (relax_column_count === true) {
          ...
          if (columns === false) {
            ...
            var _err8 = this.error("Invalid Record Length: header length is ".concat(columns.length, ", got ").concat(recordLength, " on line ").concat(this.info.lines));

            if (_err8 !== undefined) return _err8;
          }
        }
      }
