...
function (_Transform) {
  ...
  _createClass(Parser, [{
      ...
      if (columns === false && this.info.records === 0) {
        ...
        if (recordLength !== this.state.expectedRecordLength) {
          if (relax_column_count === true) {
            ...
            if (columns === false) {
              var err = this.__error("Invalid Record Length: expect ".concat(this.state.expectedRecordLength, ", got ").concat(recordLength, " on line ").concat(this.info.lines), record);

              if (err !== undefined) return err;
            } else {
              var _err5 = this.__error("Invalid Record Length: header length is ".concat(columns.length, ", got ").concat(recordLength, " on line ").concat(this.info.lines), record);
    ...
    value: function __firstLineToColumns(record) {
      ...
      try {
        ...
        if (!Array.isArray(headers)) {
          return this.__error("Invalid Header Mapping: expect an array, got ".concat(JSON.stringify(headers)), headers);
        }
    ...
    key: "__error",
    value: function __error(msg, additionalData) {
      var skip_lines_with_error = this.options.skip_lines_with_error;
      var err = new Error(msg);

      if (skip_lines_with_error) {
        this.state.recordHasError = true;
        this.emit('skip', err, additionalData);
        return undefined;
      } else {
        return err;
      }
