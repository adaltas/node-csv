...
_Stringifier = function Stringifier() {
  ...
  var base, base1, base10, base11, base12, base13, base14, base2, base3, base4, base5, base6, base7, base8, base9, isRegExp, isString, j, k, len, options, quoted_match, ref, v; // Immutable options
  ...
  if ((base3 = this.options).quoted_empty == null) {
    base3.quoted_empty = void 0;
  }

  if ((base4 = this.options).quoted_string == null) {
    base4.quoted_string = false;
  }
  ...
  if (this.options.quoted_match === void 0 || this.options.quoted_match === null || this.options.quoted_match === false) {
    this.options.quoted_match = null;
  } else if (!Array.isArray(this.options.quoted_match)) {
    this.options.quoted_match = [this.options.quoted_match];
  }

  if (this.options.quoted_match) {
    ref = this.options.quoted_match;
  ...
  if ((base14 = this.options).row_delimiter == null) {
    base14.row_delimiter = '\n';
  ...
  switch (this.options.row_delimiter) {
    case 'auto':
      this.options.row_delimiter = null;
      break;

    case 'unix':
      this.options.row_delimiter = "\n";
      break;

    case 'mac':
      this.options.row_delimiter = "\r";
      break;
    ...
    case 'ascii':
      this.options.row_delimiter = "\x1E";
      break;
...
_Stringifier.prototype._transform = function (chunk, encoding, callback) {
  ...
  if (!preserve) {
    ...
    if (this.options.eof) {
      ...
      chunk = chunk + this.options.row_delimiter;
    } else {
      chunk = this.stringify(chunk);
      ...
      if (this.options.header || this.countWriten) {
        chunk = this.options.row_delimiter + chunk;
      }
    }
  }
  ...
  if (Array.isArray(record)) {
    ...
    for (i = l = 0, ref1 = record.length; 0 <= ref1 ? l < ref1 : l > ref1; i = 0 <= ref1 ? ++l : --l) {
      ...
      if (field) {
        ...
        containsRowDelimiter = field.indexOf(this.options.row_delimiter) >= 0;
...
_Stringifier.prototype.headers = function () {
  ...
  if (this.options.eof) {
    headers = this.stringify(headers) + this.options.row_delimiter;
  } else {
