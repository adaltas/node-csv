...
// information.
var Stringifier, bom_utf8, castPath, charCodeOfDot, get, getTag, isKey, isObject, isSymbol, reEscapeChar, reIsDeepProp, reIsPlainProp, rePropName, stream, stringToPath, toKey, underscore, util;
stream = require('stream');
util = require('util');
bom_utf8 = Buffer.from([239, 187, 191]); // ## Usage
...
function (_stream$Transform) {
  ...
  function Stringifier() {
    ...
    if (options.bom == null) {
      options.bom = false;
    } else if (typeof options.bom !== 'boolean') {
      throw Error(['OPTION_BOOLEAN_INVALID_TYPE:', 'option `bom` is optional and must be a boolean value,', "got ".concat(JSON.stringify(options.bom))].join(' '));
  ...
  _createClass(Stringifier, [{
    ...
    value: function stringify(chunk) {
      ...
      var column, columns, containsEscape, containsQuote, containsRecordDelimiter, containsdelimiter, csvrecord, delimiter, err, escape, field, header, i, j, l, len, m, options, quote, quoted, quotedMatch, quotedString, quoted_empty, quoted_match, quoted_string, record, record_delimiter, ref, ref1, regexp, shouldQuote, value;
      ...
      for (i = m = 0, ref1 = record.length; 0 <= ref1 ? m < ref1 : m > ref1; i = 0 <= ref1 ? ++m : --m) {
        ...
        if (value) {
          ...
          shouldQuote = containsQuote === true || containsdelimiter || containsRecordDelimiter || quoted || quotedString || quotedMatch;
