...
var Parser = /*#__PURE__*/function (_Transform) {
  ...
  _createClass(Parser, [{
    ...
    value: function __parse(nextBuf, end) {
      ...
      if (bomSkipped === false) {
        if (bom === false) {
          ...
          for (var encoding in boms) {
            console.log(encoding, boms[encoding].compare(buf, 0, boms[encoding].length), boms[encoding], buf);
      ...
      for (pos = 0; pos < bufLen; pos++) {
        ...
        if (this.state.escaping === true) {
          ...
          if (escape !== null && this.state.quoting === true && this.__isEscape(buf, pos) && pos + escape.length < bufLen) {
            if (escapeIsQuote) {
          ...
          if (this.state.commenting === false && this.__isQuote(buf, pos)) {
            if (this.state.quoting === true) {
              ...
              var isNextChrDelimiter = this.__isDelimiter(nextChr, buf, pos + quote.length);
              ...
              if (escape !== null && this.__isEscape(buf, pos) && this.__isQuote(buf, pos + escape.length)) {
                pos += escape.length - 1;
    ...
    value: function __needMoreData(i, bufLen, end) {
      ...
      quoting ? quote.length + recordDelimiterMaxLength : 0, // Skip if the remaining buffer can be delimiter
    ...
    key: "__isEscape",
    value: function __isEscape(buf, pos) {
