          ...
          if (this.state.commenting === false && chr === quote) {
            if (this.state.quoting === true) {
              ...
              if (this.state.field.length !== 0) {
                ...
                if (relax === false) {
                  var _err = this.__error(new CsvError('INVALID_OPENING_QUOTE', ['Invalid Opening Quote:', "a quote is found inside a field at line ".concat(this.info.lines)], {
                    field: JSON.parse(JSON.stringify(this.state.field))
                  }));
