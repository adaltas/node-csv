...
Parser.prototype.__flush = function () {
  ...
  if (this._.quoting) {
    err = Error('Quoted field not terminated at line ' + (this.lines + 1));
...
Parser.prototype.__push = function (line) {
  ...
  if (line.length === 1 && line[0] === '') {
    ...
    if (this.options.relax_column_count) {
      ...
      err = Error('Number of columns on line ' + this.lines + ' does not match header');
      if (!this.options.skip_lines_with_error) {
        return err;
      } else {
        this.emit('skip', err);
      ...
      err = Error('Number of columns is inconsistent on line ' + this.lines);
      if (!this.options.skip_lines_with_error) {
        return err;
      } else {
        this.emit('skip', err);
      ...
      if (this._.quoting) {
        ...
        if (this._.nextChar && !areNextCharsRowDelimiters && !areNextCharsDelimiter && !isNextCharAComment) {
          if (this.options.relax) {
            ...
            err = Error('Invalid closing quote at line ' + (this.lines + 1) + '; found ' + JSON.stringify(this._.nextChar) + ' instead of delimiter ' + JSON.stringify(this.options.delimiter));
            if (!this.options.skip_lines_with_error) {
              return err;
            } else {
        ...
        err = Error('Invalid opening quote at line ' + (this.lines + 1));
        if (!this.options.skip_lines_with_error) {
          return err;
        } else {
          if (!this._.lineHasError) {
