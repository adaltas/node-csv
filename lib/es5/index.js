  ...
  _createClass(Parser, [{
    ...
    value: function __parse(nextBuf, end) {
      ...
      for (pos = 0; pos < bufLen; pos++) {
        ...
        var recordDelimiterLength = this.__isRecordDelimiter(chr, buf, pos); // if(from_line !== 1 && this.info.lines < this.options.from_line){
        //   pos += recordDelimiterLength === 0 ? 0 : recordDelimiterLength - 1
        //   continue
        // }
        ...
        if (this.state.escaping === true) {
          ...
          if (this.state.quoting === false) {
            if (recordDelimiterLength !== 0) {
              ...
              continue; // if(recordDelimiterLength !== 0 && this.state.enabled === false && this.info.lines >= from_line){
              //   this.state.enabled = true
              //   this.__resetField()
              //   this.__resetRow()
              // }else{
