...
Parser.prototype.parse = function(chars) {
  var areNextCharsRowDelimiters, char, csv, escapeIsQuote, i, isEscape, isQuote, isReallyEscaped, l, nextChar;
  csv = this.csv;
  chars = '' + chars;
  l = chars.length;
  i = 0;
  ...
  while (i < l) {
    char = chars.charAt(i);
    char = nextChar ? nextChar : chars.charAt(i);
    nextChar = chars.charAt(i + 1);
    if (!(this.options.rowDelimiter != null) && (nextChar === '\n' || nextChar === '\r')) {
      this.options.rowDelimiter = nextChar;
      if (nextChar === '\r' && chars.charAt(i + 2) === '\n') {
    ...
    if (char === this.options.escape || char === this.options.quote) {
      ...
      if (!isReallyEscaped && char === this.options.quote) {
        if (this.quoting) {
          ...
          if (nextChar && !areNextCharsRowDelimiters && nextChar !== this.options.delimiter) {
