...
Parser.prototype.write = function(chars, end) {
  var areNextCharsRowDelimiters, char, csv, delimLength, escapeIsQuote, i, isEscape, isQuote, isReallyEscaped, l, ltrim, rtrim, _results;
  csv = this.csv;
  ...
  while (i < l) {
    ...
    if (!this.quoting && char === this.options.delimiter) {
      if (rtrim) {
        this.field = this.field.trimRight();
      }
      this.line.push(this.field);
      this.field = '';
    } else if (!this.quoting && (this.options.rowDelimiter && chars.substr(i, this.options.rowDelimiter.length) === this.options.rowDelimiter)) {
      this.lines++;
      if (rtrim) {
        this.field = this.field.trimRight();
      }
