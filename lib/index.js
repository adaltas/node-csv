...
Parser.prototype._transform = function(chunk, encoding, callback) {
  ...
  this.__write(chunk, false, callback);
  return callback();
};
...
Parser.prototype._flush = function(callback) {
  this.__write('', true);
  if (this.quoting) {
    return callback(new Error("Quoted field not terminated at line " + (this.lines + 1)));
  }
    ...
    if (char === this.options.quote) {
      if (this.quoting) {
        ...
        if (!this.options.relax && this.nextChar && !areNextCharsRowDelimiters && this.nextChar !== this.options.delimiter && this.nextChar !== this.options.comment) {
          return callback(new Error("Invalid closing quote at line " + (this.lines + 1) + "; found " + (JSON.stringify(this.nextChar)) + " instead of delimiter " + (JSON.stringify(this.options.delimiter))));
        }
        this.quoting = false;
        this.closingQuote = i;
        i++;
