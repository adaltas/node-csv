...
CSV = (function(_super) {
  ...
  function CSV() {
    ...
    this.parser.on('row', (function(_this) {
      return function(row) {
        return _this.transformer.write(row);
      };
    })(this));
    this.parser.on('end', (function(_this) {
      return function() {
        if (_this.state.count === 0) {
          _this.transformer.headers();
        }
        return _this.transformer.end();
      };
    })(this));
    ...
    this.transformer.on('end', (function(_this) {
      return function() {
        var eof;
        eof = _this.options.to.eof;
        if (eof) {
          if (eof === true) {
            eof = '\n';
          }
          _this.stringifier.write(eof);
        }
        return _this.emit('end', _this.state.count);
      };
    })(this));
  }
