...
class Parser extends Transform {
  constructor(opts = {}){
    ...
    if(options.comment === undefined || options.comment === null || options.comment === false || options.comment === ''){
      ...
      if(!Buffer.isBuffer(options.comment)){
        throw new Error(`Invalid Option: comment must be a buffer or a string, got ${JSON.stringify(options.quote)}`)
      }
    }
  ...
  __parse(nextBuf, end){
    const {comment, escape, from, ltrim, max_limit_on_data_read, quote, raw, relax, rtrim, skip_empty_lines, to} = this.options
    ...
    for(pos = 0; pos < bufLen; pos++){
      ...
      if(commenting === false && max_limit_on_data_read !== 0){
        if(this.state.field.length > max_limit_on_data_read){
          const err = this.error(`Field exceeds max_limit_on_data_read: maximum value is ${max_limit_on_data_read}`)
          if(err !== undefined) return err
        }
        if(this.state.record_length + this.state.field.length > max_limit_on_data_read){
          const err = this.error(`Record exceeds max_limit_on_data_read: maximum value is ${max_limit_on_data_read}`)
          if(err !== undefined) return err
        }
      }
