...
Parser = function(options) {
  ...
  if ((base13 = this.options).max_limit_on_data_read == null) {
    base13.max_limit_on_data_read = false;
  }
  this.lines = 0;
  this.count = 0;
  this.is_int = /^(\-|\+)?([1-9]+[0-9]*)$/;
...
Parser.prototype.__write = function(chars, end, callback) {
  var acceptedLength, areNextCharsDelimiter, areNextCharsRowDelimiters, auto_parse, char, escapeIsQuote, i, isDelimiter, isEscape, isNextCharAComment, isQuote, isRowDelimiter, is_float, is_int, l, ltrim, maxLimitOnDataRead, nextCharPos, ref, results, rowDelimiter, rowDelimiterLength, rtrim, wasCommenting;
  ...
  l = chars.length;
  maxLimitOnDataRead = this.options.max_limit_on_data_read ? this.options.max_limit_on_data_read : 128000;
  ...
  while (i < l) {
    ...
    if (!this.commenting && this.field.length > maxLimitOnDataRead) {
      throw Error("Delimter not found in the file " + (JSON.stringify(this.options.delimiter)));
    }
    if (!this.commenting && this.line.length > maxLimitOnDataRead) {
