...
Parser.prototype.__write = function(chars, end, callback) {
  var MAX_CHAR_READ, acceptedLength, areNextCharsDelimiter, areNextCharsRowDelimiters, auto_parse, char, escapeIsQuote, i, isDelimiter, isEscape, isNextCharAComment, isQuote, isRowDelimiter, is_float, is_int, l, ltrim, nextCharPos, ref, results, rowDelimiter, rowDelimiterLength, rtrim, wasCommenting;
  ...
  while (i < l) {
    ...
    if (!this.commenting && this.field.length > MAX_CHAR_READ) {
      throw Error("Delimter not found in the file " + (JSON.stringify(this.options.delimiter)));
    }
    if (!this.commenting && this.line.length > MAX_CHAR_READ) {
