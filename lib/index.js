...
class Parser extends Transform {
  constructor(opts = {}){
    ...
    if(options.delimiter === undefined || options.delimiter === null || options.delimiter === false){
    ...
    }else if(Array.isArray(options.delimiter) && options.delimiter.length !== 0){
      options.delimiter = options.delimiter.map(function(c){
        if(c.length !== 0){
          if(typeof c === 'string'){
      ...
      (Array.isArray(options.delimiter) && options.delimiter.length === 0) ||
      (!Buffer.isBuffer(options.delimiter) && typeof options.delimiter !== 'string' && !Array.isArray(options.delimiter))
    ){
      throw delimiter_error
    }
  ...
  __isDelimiter(chr, buf, pos, delimiter = this.options.delimiter){
    const delLength = delimiter.length
    if(Array.isArray(delimiter)){
      for(let i = 0; i < delLength; i++){
        const result = this.__isDelimiter(chr, buf, pos, delimiter[i]);
