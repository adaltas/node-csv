...
class Parser extends Transform {
  constructor(opts = {}){
    ...
    let delimiter_error = new CsvError('CSV_INVALID_OPTION_DELIMITER', [
      ...
      `got ${JSON.stringify(delimiter)}`
    ])
    if(!Array.isArray(delimiter)) delimiter = [delimiter]
    if(delimiter.length === 0) throw delimiter_error
    options.delimiter = delimiter.map(function(del){
